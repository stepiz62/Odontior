...
public class InvoiceListFragment extends JotyResultFragment {
    ...
    protected JotyResourceCursorAdapter createAdapter(JotyCursorWrapper cursorWrapper) {
        return  new JotyResourceCursorAdapter(getActivity(), R.layout.row_invoice, cursorWrapper, 0){
            @Override
            public void bindView(View view, Context context, Cursor cursor) {
                TextView number = (TextView) view.findViewById(R.id.number);
                number.setText(String.valueOf(cursor.getLong(cursor.getColumnIndex("number"))));

                TextView date = (TextView) view.findViewById(R.id.date);
                date.setText(((JotyCursorWrapper)cursor).getDate("date").render());
